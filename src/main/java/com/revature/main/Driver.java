...
//                  System.out.println(ad.getAllAccounts(activeUser));
              ...
              case "TxA":
                  if(args.length>1){
                    ...
                    if(test.getAccountID() == activeUser.getUserID() || activeUser.isAdmin()){
                      System.out.println(tx.getTransactionsForUser(accid));
                    }
                    else{
                      System.out.println("This account does not belong to you");
                    }
                  }
                  else{System.out.println("Please Specify an account");}
                  break;
              case "TxU":
                  if(!activeUser.isAdmin()){
                    ...
                    if(args.length>1){
                      ...
                      System.out.println(tx.getTransactionsForUser(uid));
                    }
                    else{System.out.println(tx.getTransactionsForUser(activeUser.getUserID()));}
                  }
                  break;
              case "new":
                  if (activeUser.isAdmin()) {
        ...
        }catch (Exception e){System.out.println("Invalid user input, use 'help' to see valid commands.");}
    }
